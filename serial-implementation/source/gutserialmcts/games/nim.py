...
class NimState(GameState):
    """
        ...
        In this game two players alternately take turns to remove _chips from stack - they can only remove
        (1,2, or 3) chip at once. The player to take last chip wins.
    """

    def __init__(self, last_active_player=2, _chips=15):
        super().__init__(last_active_player, _chips)

    def clone(self):
        """
            Return a deep clone of self
        """
        return NimState(self.last_active_player, self._chips)

    def perform_action(self, action):
        """
            Perform given action and change last_active_player
            :param action: action to be performed - in this case, number of _chips to be removed
        """
    ...
    def get_available_actions(self):
        """
            ...
            In this case: you can take up to 3 _chips
        """
        return list(range(1, min([4, self._chips + 1])))
    ...
    def __repr__(self):
        ...
        """
        return "Last active player: {0}; Chips: {1}".format(self.last_active_player, self._chips)
